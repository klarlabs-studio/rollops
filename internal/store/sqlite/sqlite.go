...
//go:embed migrations/0005_db_migrate_when.sql
...
//go:embed migrations/0008_analysis.sql
...
//go:embed migrations/0011_stepper_snap.sql
var migration0011 string

//go:embed migrations/0012_rollback_blocked.sql
...
func Open(path string) (*Store, error) {
	...
	if err := applyAddColumns(db, migration0011); err != nil {
		_ = db.Close()
		return nil, fmt.Errorf("sqlite: migrate 0011: %w", err)
	}
	if err := applyAddColumns(db, migration0012); err != nil {
		_ = db.Close()
		return nil, fmt.Errorf("sqlite: migrate 0012: %w", err)
	}
	return &Store{db: db}, nil
}

// applyAddColumns runs each statement in an ADD COLUMN migration separately,
// ignoring "duplicate column" so re-running on an existing database is a no-op.
func applyAddColumns(db *sql.DB, migration string) error {
	// Drop comment lines first, so a stray ';' inside a comment can't split a
	// statement, then execute each ';'-terminated statement on its own.
	var sb strings.Builder
	for line := range strings.SplitSeq(migration, "\n") {
		if l := strings.TrimSpace(line); l != "" && !strings.HasPrefix(l, "--") {
			sb.WriteString(l)
			sb.WriteByte('\n')
		}
	}
	for chunk := range strings.SplitSeq(sb.String(), ";") {
		stmt := strings.TrimSpace(strings.ReplaceAll(chunk, "\n", " "))
		if stmt == "" {
			continue
		}
...
func (s *Store) SaveRollout(ctx context.Context, r rollout.Rollout) error {
	...
	dbCmd, err := encodeDBCmd(r.DBRollbackCmd)
	if err != nil {
	...
	_, err = tx.ExecContext(ctx, `
		INSERT INTO rollouts (id, target_ref, phase, strategy, manifest, risk_score, initiator_kind, initiator_name, step_index, step_total, step_weight, db_rollback_cmd, db_rollback_timeout, db_migrate_cmd, db_backward_compatible, db_migrate_timeout, db_migrate_when, delivery_traffic, delivery_flag, analysis, smoke_test, stepper_snap, rollback_blocked, note, created_at, updated_at)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
		ON CONFLICT(id) DO UPDATE SET
			...
			analysis=excluded.analysis, smoke_test=excluded.smoke_test,
			stepper_snap=excluded.stepper_snap, rollback_blocked=excluded.rollback_blocked,
		...
		string(r.DeliveryTraffic), string(r.DeliveryFlag), string(r.Analysis), string(r.SmokeTest),
		string(r.StepperSnap), r.RollbackBlocked,
...
// encodeDBCmd marshals a database rollback argv to a JSON column value; an empty
// command stores "" so the no-hook case stays a plain empty string.
func encodeDBCmd(cmd []string) (string, error) {
	if len(cmd) == 0 {
		return "", nil
	}
...
func (s *Store) LoadRollout(ctx context.Context, id string) (rollout.Rollout, error) {
	var (
		...
		analysis, smokeTest           string
		stepperSnap                   string
		created, updated              string
	)
	err := s.db.QueryRowContext(ctx, `
		SELECT id, target_ref, phase, strategy, manifest, risk_score, initiator_kind, initiator_name, step_index, step_total, step_weight, db_rollback_cmd, db_rollback_timeout, db_migrate_cmd, db_backward_compatible, db_migrate_timeout, db_migrate_when, delivery_traffic, delivery_flag, analysis, smoke_test, stepper_snap, rollback_blocked, note, created_at, updated_at
		FROM rollouts WHERE id = ?`, id).
		Scan(&r.ID, &r.TargetRef, &phase, &strategy, &manifest, &r.RiskScore, &r.Initiator.Kind, &r.Initiator.Name, &r.StepIndex, &r.StepTotal, &r.StepWeight, &dbCmd, &r.DBRollbackTimeout, &migCmd, &r.DBBackwardCompatible, &r.DBMigrateTimeout, &r.DBMigrateWhen, &deliveryTraffic, &deliveryFlag, &analysis, &smokeTest, &stepperSnap, &r.RollbackBlocked, &r.Note, &created, &updated)
	...
	r.Analysis = blobBytes(analysis)
	r.SmokeTest = blobBytes(smokeTest)
	r.StepperSnap = blobBytes(stepperSnap)
...
func (s *Store) ListRollouts(ctx context.Context, limit int) ([]rollout.Rollout, error) {
	q := `SELECT id, target_ref, phase, strategy, manifest, risk_score, initiator_kind, initiator_name, step_index, step_total, step_weight, db_rollback_cmd, db_rollback_timeout, db_migrate_cmd, db_backward_compatible, db_migrate_timeout, db_migrate_when, delivery_traffic, delivery_flag, analysis, smoke_test, stepper_snap, rollback_blocked, note, created_at, updated_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.TargetRef, &phase, &strategy, &manifest, &r.RiskScore,
			&r.Initiator.Kind, &r.Initiator.Name, &r.StepIndex, &r.StepTotal, &r.StepWeight, &dbCmd, &r.DBRollbackTimeout, &migCmd, &r.DBBackwardCompatible, &r.DBMigrateTimeout, &r.DBMigrateWhen, &deliveryTraffic, &deliveryFlag, &analysis, &smokeTest, &stepperSnap, &r.RollbackBlocked, &r.Note, &created, &updated); err != nil {
