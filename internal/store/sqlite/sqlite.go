...
func (s *Store) SaveRollout(ctx context.Context, r rollout.Rollout) error {
	...
	_, err = tx.ExecContext(ctx, `
		...
		ON CONFLICT(id) DO UPDATE SET
			...
			stepper_snap=excluded.stepper_snap,
...
func (s *Store) ListRollouts(ctx context.Context, limit int) ([]rollout.Rollout, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.TargetRef, &phase, &strategy, &manifest, &r.RiskScore,
			&r.Initiator.Kind, &r.Initiator.Name, &r.StepIndex, &r.StepTotal, &r.StepWeight, &dbCmd, &r.DBRollbackTimeout, &migCmd, &r.DBBackwardCompatible, &r.DBMigrateTimeout, &r.DBMigrateWhen, &deliveryTraffic, &deliveryFlag, &analysis, &smokeTest, &stepperSnap, &r.Note, &created, &updated); err != nil {
