...
// closeTarget releases a binding's runtime resources (a plugin subprocess);
// no-op for a target that holds nothing. Engine operations defer this after
// every build.
func closeTarget(b *targetv2.Bound) { _ = b.Close() }
...
func (e *Engine) build(t config.Target) (*targetv2.Bound, error) {
...
// desiredOf converts a stored manifest to the desired state the contract
// carries. Root does not survive: it is the path of a local checkout, which
// says how the bytes were reached rather than what is wanted, and the manual
// rollback path has no checkout at all. Rendered does survive — it is what the
// pointer already resolved to, and it is the only way a rollback restores the
// bytes that were deployed rather than whatever the source says today.
func desiredOf(m pt.Manifest) targetv2.DesiredState {
...
// applyOf builds the apply request for one intent. The key pairs the rollout
// with the checksum of what it is applying, because §9.4 exists for the retry
// that follows a lost response: a key minted at call time is regenerated by
// that retry and defeats itself, where both of these were durable before the
// call. The checksum is what separates a deploy from the rollback that follows
// it in the same rollout — two intents, and replaying one as the other would
// put the wrong release back.
func applyOf(rolloutID string, m pt.Manifest) targetv2.ApplyRequest {
	return targetv2.ApplyRequest{
		Desired:        desiredOf(m),
		IdempotencyKey: targetv2.IdempotencyKeyFor(rolloutID, m.Checksum),
	}
}
...
//
// Plan is mandatory, so every target is asked and none are skipped. A target
// that reports no blockers is saying "nothing is known to block this", which is
// weaker than "this will succeed" — a target with no checks of its own is
// indistinguishable here from one whose checks all passed, and neither promises
// the apply works.
func (e *Engine) Preflight(ctx context.Context, cs []*config.Config) []error {
	...
	for _, c := range cs {
		...
		res, err := e.stampReferencedChecksum(ctx, tgt, &m)
		closeTarget(tgt)
		if err != nil {
		...
		for _, blocker := range res.Blockers {
			out = append(out, fmt.Errorf("%s: %s", c.Spec.Target.Ref, blocker))
		}
	}
	return out
}
...
func (e *Engine) Plan(ctx context.Context, c *config.Config) (*Plan, error) {
	...
	// rendered bytes double as the plan preview, and the same plan answers the
	// live-diff question below.
	plan, planErr := e.stampReferencedChecksum(ctx, tgt, &m)
	obs, err := tgt.Observe(ctx, targetv2.ObserveRequest{})
	if err != nil {
		return nil, fmt.Errorf("engine: plan: observe: %w", err)
	}
	cur := pt.Fingerprint{Value: obs.Fingerprint, Meta: obs.Meta}
	...
	// The live diff costs nothing extra: the plan that re-keyed the checksum
	// already computed it, so this reads a result rather than asking again.
	...
	if (ver == "detect" || ver == "full") && cur.Value == m.Checksum && m.Checksum != "" {
		// The diff comes off the plan above. It used to need a second,
		// undecorated build because the resilience envelope dropped the optional
		// verbs; capabilities now travel as data, so there is one target, one
		// process and one plan.
		switch {
		case planErr != nil:
		...
		case strings.TrimSpace(plan.Diff) != "":
			if ver == "full" {
				deepDrift = true
			} else {
				driftAlert = true
	...
	// The preview is the bytes the rollout will carry, which the stamp sets only
	// when the desired checksum was keyed over a pointer. An inline manifest is
	// its own preview and has nothing to show here.
	p.Rendered = m.Rendered
...
func (e *Engine) Apply(ctx context.Context, req ApplyRequest) (*rollout.Rollout, error) {
	...
	if err := e.deployOnce(ctx, cfg, tgt, r.ID, m); err != nil {
...
func (e *Engine) Approve(ctx context.Context, rolloutID string, by rollout.Identity) (rollout.Rollout, error) {
	...
	defer closeTarget(tgt)
	if _, err := tgt.Apply(ctx, applyOf(r.ID, r.Desired)); err != nil {
...
func (e *Engine) Observe(ctx context.Context, t config.Target) (pt.Fingerprint, error) {
	...
	defer closeTarget(tgt)
	obs, err := tgt.Observe(ctx, targetv2.ObserveRequest{})
	fp := pt.Fingerprint{Value: obs.Fingerprint, Meta: obs.Meta}
	if err != nil {
...
func (e *Engine) applyRollback(ctx context.Context, r *rollout.Rollout, prior pt.Manifest, note string, db *config.DatabaseRollback, force bool) (rollout.Rollout, error) {
	...
	defer closeTarget(tgt)
	if _, err := tgt.Apply(ctx, applyOf(r.ID, prior)); err != nil {
...
// rawTarget builds a target without the resilience envelope, for the read-only
// paths that have nothing to retry.
func (e *Engine) rawTarget(ref string, m pt.Manifest) (*targetv2.Bound, error) {
...
func (e *Engine) Diff(ctx context.Context, rolloutID string) (string, error) {
	...
	defer closeTarget(tgt)
	if !tgt.Can(targetv2.CapabilityDriftDetection) {
		return "", ErrUnsupported
	}
	res, err := tgt.Plan(ctx, targetv2.PlanRequest{Desired: desiredOf(r.Desired)})
	if err != nil {
		return "", err
	}
	return res.Diff, nil
...
// Resources lists the live resources a rollout's target manages. Inspect is
// mandatory in v2, so a target with no inventory to report answers with an
// empty one rather than refusing — "this target manages nothing it can name" is
// a true answer, where the old ErrUnsupported forced every caller to hold a
// branch for a question that no longer has two outcomes.
func (e *Engine) Resources(ctx context.Context, rolloutID string) ([]pt.Resource, error) {
	...
	out := make([]pt.Resource, 0, len(state.Resources))
	for _, res := range state.Resources {
		out = append(out, pt.Resource{
			Kind:      res.Kind,
			Name:      res.Name,
...
func (e *Engine) applyScheduled(ctx context.Context, s rollout.ScheduledRollout) (rollout.Rollout, error) {
	...
	// The key is derived from the scheduled row rather than from r, whose ID is
	// minted a few lines above: a retry that re-enters this function mints a new
	// one, which is exactly the self-defeating key §9.4 exists to avoid.
	if _, err := tgt.Apply(ctx, applyOf(s.ID, s.Desired)); err != nil {
...
func (e *Engine) buildTarget(ref string, m pt.Manifest) (*targetv2.Bound, error) {
	var spec map[string]any
	if len(m.Spec) > 0 {
		if err := json.Unmarshal(m.Spec, &spec); err != nil {
			return nil, fmt.Errorf("engine: decode manifest spec: %w", err)
...
// stampReferencedChecksum re-keys a manifest's drift checksum over the bytes
// the target will actually apply, when the plan says the desired checksum does
// not already identify them. A spec that points at an external source is
// checksummed over the pointer, so editing the files behind it leaves that
// checksum untouched and the drift check sees nothing; keying off the plan's
// own checksum makes the edit surface even under shallow verification. A spec
// that carries its own bytes reports no second checksum and keeps the one it
// has.
//
// It hands the plan back rather than keeping it, because v2 answers "what would
// change", "what would be sent" and "what would refuse this" in one verb where
// v1 had three. A caller that wants any of the other two must otherwise ask for
// all three again, and this runs once per in-sync target per reconcile tick.
// Plan is side-effect free (§9.5), so asking twice would be safe — it would
// just double the render, the diff and the preflight on the hot path.
func (e *Engine) stampReferencedChecksum(ctx context.Context, tgt *targetv2.Bound, m *pt.Manifest) (targetv2.PlanResult, error) {
	res, err := tgt.Plan(ctx, targetv2.PlanRequest{Desired: desiredOf(*m)})
	if err != nil || res.RenderedChecksum == "" {
		return res, err
	}
	m.Checksum = res.RenderedChecksum
	...
	m.Rendered = res.Rendered
	return res, nil
}

// stampVia does the same for a caller that has no target in hand. A target that
// will not build is not reported here: it fails louder a few lines later, from
// the caller that needed it for more than a checksum.
func (e *Engine) stampVia(ctx context.Context, ref string, m *pt.Manifest) {
	raw, err := e.rawTarget(ref, *m)
	if err != nil {
		return
	}
	defer closeTarget(raw)
	_, _ = e.stampReferencedChecksum(ctx, raw, m)
}
...
// nothing else.
func (e *Engine) BuildTarget(t config.Target) (*targetv2.Bound, error) {
